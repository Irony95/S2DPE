...
import physicsComponents.EntityUnit;
import physicsComponents.PolygonCollider;
...
public class ImpulseSolver {
	...
	public static RealVector[] solveImpulse(EntityUnit objA, EntityUnit objB, RealVector normal,
		...
		RealVector relativeB = rB == null ? objB.velocity : objB.velocity.add(VecMath.cross(rB, colB.getAngularVelocity()));
		RealVector relativeA = rA == null ? objA.velocity : objA.velocity.add(VecMath.cross(rA, colA.getAngularVelocity()));
		RealVector rv = relativeB.subtract(relativeA);

		double velAlongNormal = rv.dotProduct(normal);
		if (velAlongNormal > 0) { return null; }
		
		double rACrossN = rA == null ? 0 : VecMath.cross(rA, normal);
		double rBCrossN = rB == null ? 0 : VecMath.cross(rB, normal);
		...
		relativeB = rB == null ? objB.velocity : objB.velocity.add(VecMath.cross(rB, colB.getAngularVelocity()));
		relativeA = rA == null ? objA.velocity : objA.velocity.add(VecMath.cross(rA, colA.getAngularVelocity()));
	...
	public static RealVector[] solveImpulse(EntityUnit obj, PolygonCollider col, RealVector normal, RealVector rA) {
		...
		if (rA != null) { rv.add(VecMath.cross(rA, col.getAngularVelocity())); }
		double velAlongNormal = rv.dotProduct(normal);
